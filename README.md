...
## Storefront App Proxy and Theme Block

1. Enable App Proxy
   - In Shopify Admin → Apps → App setup → App proxy
   - Set Subpath prefix to `apps`, Subpath to `ring-builder`
   - Point to your tunnel base: `<your tunnel>/proxy`
   - Save. Your storefront proxy URL will be `/apps/ring-builder/*`

2. Remix proxy route
   - We expose the grid at `/apps/ring-builder/grid` via `app/routes/proxy.grid.tsx` which re-exports `app.grid`.

3. Theme Editor: add the block
   - Go to Online Store → Themes → Customize
   - Add section/block: “Ring Builder Grid (App)”
   - Configure Height (px) if needed and ensure the App proxy base is `/apps/ring-builder`

4. Local dev tips
   - Start dev server: `npm run dev`
   - If routes are newly created, restart dev server
   - Confirm storefront loads iframe at `/apps/ring-builder/grid`
